...
// This is just Crockford's json2.js minified by the Google Closure compiler. Nothing special.
...
$(document).ready(function() {
	...
	$("#grade-table").tablesorter({sortList: [[2,0], [1,0]]}); 
	$("#student-table").tablesorter({sortList: [[0,0]]});
	$("#courses-table").tablesorter({sortList: [[1,0]]});
	$("#students-table").tablesorter({sortList: [[3,0], [4,0], [1,0], [0,0]]});
	$("#probation-table").tablesorter({sortList: [[2,0]]});
	$("#movement-table").tablesorter({sortList: [[3,0]]});
	$("#feeder-schools").tablesorter({sortList: [[8,1]]});
	$(".report-table").tablesorter({sortList: [[2,0]]});
		
	// Turns on the dropdown menu on the top bar, duh
	$('#topbar').dropdown();
		
	if ($('heading h1').text().match(/English/)) {hideMath();}
});
